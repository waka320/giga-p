...
アクロバスター.（AcroBuster.）は、IT分野で使用されるアクロニム（略語）を題材にしたワードパズルゲームです。プレイヤーは制限時間内に、画面上に配置されたアルファベットを組み合わせ、HTTPやCSSといったIT用語のアクロニムを見つけ出し、高得点を目指します。
...
アクロバスター.はReact Context APIを使用した状態管理アーキテクチャを採用しています。これにより、プロップドリリングを避けつつ、複数のコンポーネント間でゲーム状態を共有できます。
...
アクロバスター.は明確な責任分担を持つクライアント-サーバーアーキテクチャを採用しています。これにより、保守性が高く拡張しやすい構造になっています。
...
1. **スコア計算と管理**
   - バックエンドでスコア計算を一元管理
   - フロントエンドはバックエンドから受け取ったスコアを表示

2. **グリッド状態**
   - バックエンドがグリッド生成と状態管理を担当
   - フロントエンドは視覚表現とインタラクションを担当

3. **ボーナス機能**
   - バックエンドがボーナス条件判定と計算
   - フロントエンドがボーナスメッセージの表示とアニメーション

4. **デバッグモード**
   - バックエンドの環境変数で制御
   - フロントエンドは結果を表示するのみ

この責任分担により、ビジネスロジックとプレゼンテーションの分離が実現され、保守性と拡張性が向上します.
